...
# Extract job title + link (generic method, you can make company-specific)
def extract_jobs(soup, base_url, company):
    ...
    for a in soup.find_all("a", href=True):
        ...
        # Check if any keyword is found in the job text
...
def check_jobs():
    ...
    for company, url in URLS_TO_MONITOR.items():
        ...
        if new_jobs:
            ...
            new_jobs_total.append(f"{company}:\n" + "\n".join(f"{t} - {l}" for t, l in new_jobs))
...
# Schedule daily
schedule.every(24).hours.do(check_jobs)
